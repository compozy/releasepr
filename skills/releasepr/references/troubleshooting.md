...
| `release tag ... already exists` | The derived tag exists locally or on `origin`. | For a normal release, choose the next version or stop the duplicate release. For an intentional retry after tag creation, pass `plan --allow-existing-tag`; it accepts only an annotated tag at the planned commit and never overwrites it. |
| `release tag ... exists but is not annotated` / `release tag ... resolves to ..., expected ...` | Recovery found an unsafe tag shape or a tag at a different commit. | Stop the retry and inspect the tag. Do not replace or overwrite it through pr-release. |
