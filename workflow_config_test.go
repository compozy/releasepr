...
func TestReleaseWorkflowConfig(t *testing.T) {
	...
	t.Run("Should dispatch release pull request checks after automation updates", func(t *testing.T) {
		...
		orchestratorStep := findStepByName(t, releasePR, "Run PR Release Orchestrator")
		orchestratorID := mappingValue(orchestratorStep, "id")
		require.NotNil(t, orchestratorID)
		assert.Equal(t, "pr_release", orchestratorID.Value)
		orchestratorRun := mappingValue(orchestratorStep, "run")
		require.NotNil(t, orchestratorRun)
		assert.Contains(t, orchestratorRun.Value, "has_release_pr=true")
		assert.Contains(t, orchestratorRun.Value, "has_release_pr=false")
		assert.Contains(t, orchestratorRun.Value, "release_branch=$branch")

		dispatchStep := findStepByName(t, releasePR, "Dispatch Release PR Checks")
		dispatchIf := mappingValue(dispatchStep, "if")
		require.NotNil(t, dispatchIf)
		assert.Contains(t, dispatchIf.Value, "steps.pr_release.outputs.has_release_pr == 'true'")
		dispatchEnv := mappingValue(dispatchStep, "env")
		require.NotNil(t, dispatchEnv)
		assert.Contains(t, mappingValue(dispatchEnv, "RELEASE_BRANCH").Value, "steps.pr_release.outputs.release_branch")
		dispatchRun := mappingValue(dispatchStep, "run")
		require.NotNil(t, dispatchRun)
		assert.Contains(t, dispatchRun.Value, "branch=\"$RELEASE_BRANCH\"")
